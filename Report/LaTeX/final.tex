...
\midrule
CNN Baseline & 95M & 0.5998 & 0.58 & 89.0 & Quá khớp (Overfitting) nặng \\
...
\textbf{ViT Final (scratch)} & \textbf{9M} & \textbf{0.7272} & \textbf{0.7225} & \textbf{92.91} & \textbf{Chia theo bệnh nhân} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[noitemsep]
    \item Việc chia theo ID bệnh nhân giúp loại bỏ hoàn toàn hiện tượng rò rỉ thông tin giữa các tập dữ liệu.
...
% 6.5 Thảo luận
% = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
\section{\textbf{Thảo luận (Discussion)}}

\begin{tcolorbox}[colback=yellow!5!white,colframe=yellow!75!black,title=Tóm tắt thảo luận (Discussion Summary)]
\begin{itemize}
    ...
    \item Kết quả của chúng tôi tuy thấp hơn nhưng đảm bảo độ tin cậy khoa học cao hơn.
\end{itemize}
\end{tcolorbox}
...
\subsection{\textbf{Kế hoạch triển khai (Implementation Timeline)}}

\textbf{Để hoàn thiện phần trực quan hóa kết quả, chúng tôi đề xuất lộ trình:}
\begin{enumerate}
  \item \textbf{Tuần 1}: Tạo Attention maps cho 3 trường hợp nghiên cứu điểm (Pneumonia, Cardiomegaly, Multi-label).
  \item \textbf{Tuần 2}: Vẽ đường cong PR cho đầy đủ 15 lớp bệnh lý với mô hình tốt nhất.
  \item \textbf{Tuần 3}: Phân tích định tính (Qualitative analysis) với 6-8 trường hợp tiêu biểu.
  \item \textbf{Tuần 4}: Vẽ biểu đồ hiệu chuẩn (Calibration plots) và bản đồ nhiệt ma trận nhầm lẫn (Confusion heatmaps).
\end{enumerate}

\textbf{Kết quả mong đợi (Expected impact):} Việc bổ sung các trực quan hóa này sẽ nâng cao tính thuyết phục và giá trị khoa học của báo cáo, giúp minh chứng rõ ràng khả năng của mô hình trong môi trường lâm sàng thực tế.
...
\chapter{Chi tiết triển khai PyTorch (PyTorch Implementation Details)}
	...
	\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Tóm tắt chuyển đổi]
		...
		\begin{itemize}
			...
			\item Mã nguồn sạch và có tài liệu hướng dẫn.
			\item Phong cách lập trình (coding style) nhất quán.
		\end{itemize}
	\end{tcolorbox}
	...
	\section{\textbf{Cấu trúc thư mục dự án (Repository Structure)}}
	
	\begin{lstlisting}[style=plain,caption={Cấu trúc project}]
		ViT-Chest-Xray/
		|-- Project/
...
%==============================================================================
\section{\textbf{Tổng kết các cải tiến}}

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Kết luận Chương 8 (Summary of Improvements)]
Dựa trên các thực nghiệm mở rộng, chúng tôi rút ra các kết luận quan trọng cho việc xây dựng mô hình AI cho bài toán X-quang phổi:

\begin{enumerate}
    \item \textbf{Dữ liệu là quan trọng nhất}: Sử dụng transfer learning từ ImageNet giúp tăng ít nhất 3\% AUC và giảm một nửa thời gian huấn luyện.
    \item \textbf{Cơ chế Attention cục bộ là thế mạnh}: Swin Transformer vượt trội hơn ViT truyền thống nhờ cấu trúc phân cấp và khả năng học các đặc trưng đa quy mô.
    \item \textbf{Xử lý mất cân bằng lớp bằng Focal/Asymmetric Loss}: Đặc biệt quan trọng đối với các lớp bệnh lý hiếm vốn thường bị bỏ qua trong hàm mất mát BCE tiêu chuẩn.
    \item \textbf{Sự kết hợp (Ensemble)}: Giúp mô hình đạt đến ngưỡng hiệu năng 0.92 AUC, tương đương với các công trình nghiên cứu hiện đại nhất hiện nay trên tập dữ liệu NIH ChestX-ray14.
\end{enumerate}
\end{tcolorbox}
