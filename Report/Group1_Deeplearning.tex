...
\midrule
CNN Baseline & $\sim$95M & N/A & N/A & $\sim$0.58$^*$ & 0.5998 \\
ResNet-34 & $\sim$21M & N/A & N/A & $\sim$0.53$^*$ & 0.5293 \\
ViT-v1 (scratch) & 9.0M & 0.2534 & 91.33 & 0.5854 & 0.6431 \\
ViT-v2 (scratch) & 9.0M & 0.2749 & 89.67 & 0.6303 & 0.5947 \\
\textbf{ViT-Pretrained} & $\sim$86M & 0.3768 & 87.00 & \textbf{0.6694} & N/A \\
\bottomrule
\multicolumn{6}{l}{\small $^*$Validation AUC used as proxy (no separate test evaluation in these notebooks)}
\end{tabular}
\end{table}

\textbf{Analysis of Performance Gap:} Our results show significantly lower AUC values than the original paper. This is expected due to:
\begin{enumerate}[noitemsep]
    \item \textbf{Patient-level splitting:} Prevents data leakage that may inflate metrics in image-level splits
    \item \textbf{Sample size:} Our experiments used 3,000 sample images vs. 85,000 in the original
    \item \textbf{Patch size difference:} We used 16$\times$16 patches vs. 32$\times$32 in the original ViT variants
    \item \textbf{Framework differences:} PyTorch vs TensorFlow/Keras implementations
\end{enumerate}
...
\textbf{ViT (Pre-trained)} & $\sim$86M & N/A & N/A & \textbf{0.6694} & N/A \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep]
    \item \textbf{High AUC ($>$0.8)}: Cardiomegaly (1.00 in ViT-v2), Emphysema (0.84), Infiltration (0.89 in ViT-v1)
    \item \textbf{Medium AUC (0.5-0.8)}: Effusion, Pneumothorax, Pleural Thickening
    \item \textbf{Low AUC ($<$0.5)}: Edema (0.42), Consolidation (0.33), Atelectasis (0.32), Mass (0.11 in ViT-v1)
    \item \textbf{Complementary Performance}: ViT-v1 and ViT-v2 excel at different diseases, suggesting ensemble potential
\end{itemize}

\subsection{Key Findings}
\begin{enumerate}
    \item \textbf{Pre-trained ViT achieves the best performance} (Test AUC: 0.6694), confirming the original paper's conclusion that transfer learning from ImageNet is beneficial for medical imaging.
    
    \item \textbf{ViT from scratch underperforms expected levels} (Test AUC: 0.58-0.63), suggesting that transformers require large amounts of data or pre-training to outperform CNNs.
    
    \item \textbf{CNN shows severe overfitting}: Train AUC of 0.90 vs Val AUC of 0.58 indicates the model memorizes training data without generalizing. Best validation occurs at epoch 1.
    
    \item \textbf{High accuracy does not guarantee good AUC}: Models achieve 89-91\% accuracy but only 0.53-0.67 AUC, demonstrating that accuracy is misleading for imbalanced multi-label data.
    
    \item \textbf{Class imbalance significantly affects rare disease detection}: Diseases with $<$3\% prevalence (Edema, Consolidation, Hernia) show AUC near or below 0.50 (random chance).
    
    \item \textbf{ViT-v1 and ViT-v2 show complementary strengths}: Different diseases perform best on different models, suggesting potential for ensemble methods.
\end{enumerate}
...
This study successfully replicated the comparative analysis of CNN, ResNet, and Vision Transformer architectures for multi-label chest X-ray classification. Our key contributions include:

\begin{enumerate}
    \item \textbf{Identification of methodological issues}: We discovered that the original study's image-level data splitting may cause data leakage, potentially inflating reported metrics.
    
    \item \textbf{Proper evaluation protocol}: We implemented patient-level splitting to ensure fair evaluation and real-world applicability.
    
    \item \textbf{Confirmation of ViT superiority}: Despite using proper data splitting, pre-trained ViT still outperforms other architectures, validating the original paper's main conclusion.
    
    \item \textbf{Practical recommendations}: We provide guidelines for medical image augmentation and handling class imbalance.
\end{enumerate}
