...
\graphicspath{{./}{image/}{./image/}{../../image/}{../../}}

% Include ảnh an toàn: nếu thiếu ảnh thì không làm compile fail
  ...
  \IfFileExists{#2}{%
    ...
    \begin{center}
      \fbox{\parbox{0.9\linewidth}{\centering
        \textbf{Missing image:}\\ \texttt{#2}\\
        (Hãy kiểm tra tên file/đuôi .png/.jpg và đúng thư mục \texttt{image/})
      }}
    \end{center}
  }%
}
...
% =========================
% Links (hyperref nên đặt gần cuối)
% =========================
\usepackage{xcolor}
\usepackage{hyperref}
\hypersetup{
...
\begin{titlepage}
  ...
  % Logo
  \vspace*{0.5cm}
  % pdfLaTeX KHÔNG hỗ trợ .webp. Hãy đổi fsb.webp -> fsb.png (hoặc .jpg).
  \safeincludegraphics[width=0.18\textwidth]{fsb.png}
  
  \vspace{0.8cm}

  {\Large \textbf{BÁO CÁO PHÂN TÍCH BÀI BÁO KHOA HỌC}}\\[0.8cm]

  {\LARGE \textbf{PHÂN TÍCH CHUYÊN SÂU TỪ PAPER ĐẾN CODE IMPLEMENTATION}}\\[0.6cm]
  ...
  {\large arXiv: 2406.00237}\\[0.2cm]
  {\normalsize Ananya Jain, Aviral Bhardwaj, Kaushik Murali, Isha Surani}\\
  {\normalsize University of Toronto}\\[1.2cm]
  ...
  \begin{flushleft}
    ...
    \textbf{Lớp:} MSA29HCM\\
    \textbf{Môn học:} Học sâu (Deep Learning)\\
...
\begin{table}[H]
\centering
\caption{Tổng hợp kết quả thực nghiệm từ bài báo gốc}
\label{tab:paper_results_main}
\begin{tabular}{lcccc}
\toprule
...
\section*{Phạm vi và mục tiêu của báo cáo}
...
\textbf{Từ khoá:} Chest X-ray, phân loại đa nhãn, Convolutional Neural Networks, Residual Networks, Vision Transformer, Self-Attention, Transfer Learning, Medical Image Analysis, ROC-AUC, PyTorch, Deep Learning.
...
\section{Thách thức trong chẩn đoán X-quang ngực}
...
\section{Động lực ứng dụng trí tuệ nhân tạo trong chẩn đoán hình ảnh}
...
\section{Giới thiệu bài báo gốc}
...
\begin{itemize}
  \item \textbf{Chương 2:} Phân tích bộ dữ liệu NIH ChestX-ray14.
  \item \textbf{Chương 3:} Mô hình CNN -- lý thuyết và triển khai.
  \item \textbf{Chương 4:} Mô hình ResNet.
  \item \textbf{Chương 5:} Vision Transformer và mô hình lai.
  \item \textbf{Chương 6:} Thực nghiệm và phân tích kết quả.
  \item \textbf{Chương 7:} Các cải tiến và thảo luận mở rộng.
  \item \textbf{Chương 8:} Kết luận và hướng nghiên cứu tương lai.
\end{itemize}
...
\section{Chia tập dữ liệu và nguy cơ rò rỉ dữ liệu}
...
\section{Tổng quan lý thuyết CNN}
...
\section{Phép tích chập (Convolution Operation)}
...
\section{Kiến trúc CNN trong bài báo}
...
\section{Cài đặt CNN trong PyTorch}
...
\section{Phân tích số lượng tham số}

\begin{table}[H]
\centering
\caption{Số lượng tham số của CNN baseline}
...
\section{Hàm mất mát cho bài toán đa nhãn}
...
\section{Phân tích kết quả và hạn chế}
...
\section{Minh hoạ quá trình huấn luyện}

\begin{figure}[H]
  \centering
  % SỬA typo: outout-cnn.png -> output_cnn.png
  \safeincludegraphics[width=0.75\textwidth]{output_cnn.png}
  \caption{Diễn biến AUC trên tập huấn luyện và validation của mô hình CNN baseline. Mô hình đạt AUC cao trên tập huấn luyện nhưng không cải thiện trên tập validation, cho thấy hiện tượng overfitting rõ rệt.}
  \label{fig:cnn_auc}
\end{figure}
...
\section{Động lực nghiên cứu: Degradation Problem}

\subsection{Vấn đề khi huấn luyện mạng rất sâu}
...
\subsection{Bản chất của degradation}
...
\subsection{Ý tưởng cốt lõi của ResNet}
...
\subsection{Phân tích dòng gradient}
...
\begin{table}[H]
\centering
\caption{Cấu hình huấn luyện ResNet-34}
...
\subsection{Diễn biến AUC theo epoch}

\begin{figure}[H]
\centering
\safeincludegraphics[width=0.75\textwidth]{output-resnet.png}
\caption{Diễn biến AUC huấn luyện và validation của ResNet-34}
...
\begin{table}[H]
\centering
\caption{Kết quả huấn luyện ResNet-34}
...
\subsection{Conv2d tương đương Linear Projection}
...
\subsection{Transformer Encoder Block (Pre-LN)}
...
% ============================================================
\section{Complete Vision Transformer Implementation}
...
% ============================================================
\section{Loss Function, Class Imbalance và Computational Complexity}

\subsection{Multi-label BCEWithLogitsLoss}
...
% ============================================================
\section{So sánh ViT-v1, ViT-v2 và ViT Pretrained}
...
% ============================================================
\section{Results Analysis và ROC Curves}

\subsection{ROC Curves}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{output-roc-vit-v1.png}
    \caption{ROC Curve - ViT-v1}
    \label{fig:roc_vit_v1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{output-roc-vit-v2.png}
    \caption{ROC Curve - ViT-v2}
...
\begin{table}[H]
\centering
\caption{Training hyperparameters (theo bài báo và triển khai lại)}
...
% ============================================================
\section{Main Results (Reported in the Original Paper)}

\begin{table}[H]
\centering
\caption{Performance comparison – kết quả báo cáo trong bài báo gốc}
...
\subsection{Key Observations (Paper Level)}
...
% ============================================================
\section{Reproduced and Audited Results}
...
\begin{table}[H]
\centering
\caption{Final metrics sau khi rà soát và chuẩn hoá pipeline}
...
\subsection{Why Is AUC Lower Than the Paper?}
...
% ============================================================
\section{Replication Study and Detailed Codebase Audit}
...
%----------------------------------------------------------
\subsection{Evaluation Framework for File Review}
...
%----------------------------------------------------------
\subsection{Data Acquisition: \texttt{data\_download.ipynb}}
...
%----------------------------------------------------------
\subsection{Data Processing Pipeline: \texttt{data.ipynb}}
...
%----------------------------------------------------------
\subsection{CNN Baseline: \texttt{cnn.ipynb}}
...
%----------------------------------------------------------
\subsection{Residual Network: \texttt{resnet.ipynb}}
...
%----------------------------------------------------------
\subsection{Vision Transformer from Scratch: \texttt{ViT-v1} và \texttt{ViT-v2}}
...
%----------------------------------------------------------
\subsection{Hybrid and Pretrained Models: \texttt{ViT-ResNet.ipynb}}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.75\textwidth]{output-vit-resnet.png}
\caption{Kết quả huấn luyện và đánh giá mô hình ViT-ResNet: Đường cong training loss/accuracy và ROC-AUC curve.}
...
%----------------------------------------------------------
\subsection{Overall Findings from Replication}
...
\begin{table}[h!]
\centering
\caption{So sánh hiệu suất giữa mô hình gốc và các mô hình đã cải tiến}
...
%==============================================================================
\section{Giai đoạn 1: Các cải tiến nhanh (Phase 1: Quick Wins)}
...
\begin{table}[h!]
\centering
\caption{Hiệu quả của Transfer Learning}
...
\subsubsection{Kết quả so sánh các hàm mất mát}

\begin{table}[h!]
\centering
\caption{So sánh các chiến lược xử lý mất cân bằng lớp}
...
%------------------------------------------------------------------------------
\subsection{Tăng cường dữ liệu nâng cao (Advanced Data Augmentation)}
\label{subsec:data_augmentation}

\subsubsection{Vấn đề với tăng cường dữ liệu cơ bản}
...
\subsubsection{Pipeline tăng cường dữ liệu chuyên biệt cho ảnh y khoa}
...
\subsubsection{Ablation study về tác động của tăng cường dữ liệu}

\begin{table}[h!]
\centering
\caption{Ablation study: tác động của các mức độ tăng cường dữ liệu}
...
%==============================================================================
\section{Giai đoạn 2: Cải tiến kiến trúc (Phase 2: Architecture Improvements)}
...
\begin{table}[h!]
\centering
\caption{So sánh Swin Transformer với ViT gốc}
...
%------------------------------------------------------------------------------
\subsection{Hợp nhất đặc trưng đa tỷ lệ (Multi-Scale Feature Fusion)}
...
\subsubsection{Kết quả theo từng bệnh (Per-disease)}

\begin{table}[h!]
\centering
\caption{Ảnh hưởng của multi-scale lên các loại bệnh khác nhau}
...
%==============================================================================
\section{Giai đoạn 3: Kỹ thuật nâng cao (Phase 3: Advanced Techniques)}
...
\subsubsection{Chiến lược ensemble}
...
\begin{table}[h!]
\centering
\caption{Hiệu suất của các cấu hình ensemble}
...
%------------------------------------------------------------------------------
\subsection{Định lượng bất định (Uncertainty quantification)}
...
%==============================================================================
\section{Kết luận và khuyến nghị cho các cải tiến (Conclusion)}
\label{sec:improve_conclusion}

\subsection{Tóm tắt các kết quả chính}
...
\subsection{Best practices tổng hợp}
...
%----------------------------------------------------------
\section{Architectural Insights and Theoretical Implications}
