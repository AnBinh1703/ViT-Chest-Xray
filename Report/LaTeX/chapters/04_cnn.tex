...
\end{tikzpicture}
\caption{Convolution operation: Kernel slide over input}
\end{figure}
...
Với $R_{i,j}$ là vùng pooling tại vị trí $(i,j)$.
...
\end{tikzpicture}
\caption{Max Pooling 2×2 với stride 2}
\end{figure}
...
\begin{tikzpicture}[scale=0.8, every node/.style={font=\small}]
    ...
    \draw[->] (dropout) -- (output);
\end{tikzpicture}
\caption{CNN Architecture từ paper}
\end{figure}
...
\section{Phân tích chi tiết từng layer}
...
\begin{lstlisting}[caption={Training function với AUC fix}]
def train_model(model, train_loader, val_loader, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        # Compute AUC (with fix for classes present in batch)
        all_preds = np.concatenate(all_preds)
        all_labels = np.concatenate(all_labels)
        
        # AUC Fix: Only compute for classes with both 0 and 1
        valid_classes = []
...
\section{So sánh với các architectures khác}
